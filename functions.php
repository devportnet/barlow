...
if ( ! function_exists( 'barlow_fonts_url' ) ) :
/**
 * Register Google fonts for barlow.
 *
 * Create your own barlow_fonts_url() function to override in a child theme.
 *
 * @since barlow 1.0
 *
 * @return string Google fonts URL for the theme.
 */
function barlow_fonts_url() {
	$fonts_url = '';
	$fonts     = array();
	...
	if ( 'off' !== _x( 'on', 'Barlow font: on or off', 'barlow' ) ) {
		$fonts[] = 'Barlow:400,700';
	}

	if ( 'off' !== _x( 'on', 'Lato font: on or off', 'barlow' ) ) {
		$fonts[] = 'Lato:400,700';
	}
...
function barlow_scripts() {
	...
	wp_enqueue_style( 'barlow-fonts', barlow_fonts_url(), array(), null );

	// Font Awesome
	wp_register_style( 'font-awesome', get_stylesheet_directory_uri() . '/assets/fonts/css/all.css', array(), '20220218' );
